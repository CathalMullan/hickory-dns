...
use hickory_proto::{
    ...
    dnssec::{
        MAX_RRSIGS_PER_RRSET, Proof, ProofError, ProofErrorKind, Rrset, TrustAnchors,
        find_soa_name,
        rdata::{DNSKEY, DNSSECRData, DS, RRSIG},
        verify_dnskey, verify_nsec, verify_nsec3, verify_rrset_with_dnskey, verify_rrsig_with_keys,
    },
    op::{DnsRequest, DnsRequestOptions, DnsResponse, Edns, Message, OpCode, Query},
    rr::{Name, RData, Record, RecordType, resource::RecordRef},
};
...
impl<H: DnsHandle> DnssecDnsHandle<H> {
        ...
        let mut message = match result {
            ...
            Err(err) => match err.kind {
                NetErrorKind::Proto(proto_err) => match proto_err.kind {
                    ...
                    _ => return Err(NetError::from(proto_err)),
                },
                _ => return Err(err),
            },
        };
    ...
    async fn find_ds_records(
        ...
        let zone = loop {
            ...
            match result {
                ...
                Err(NetError {
                    kind: NetErrorKind::Proto(proto_err),
                    ..
                }) if proto_err.is_no_records_found() || proto_err.is_nx_domain() => {}
                Err(e) => {
                    // FIXME(NET)
                    let proto_err = match e.kind {
                        NetErrorKind::Proto(proto_err) => proto_err,
                        _ => ProtoError::from(format!("network error during DS lookup: {e}")),
                    };

                    return Err(ProofError::new(
                        Proof::Bogus,
                        ProofErrorKind::Proto {
                            query,
                            proto: proto_err,
                        },
                    ));
    ...
    async fn fetch_ds_records(
        ...
        if let Some(NetErrorKind::Proto(ProtoError {
            kind: ProtoErrorKind::Dns(DnsError::Nsec { query, proof, .. }),
            ..
                        ...
                        .map(move |result| match result {
                            ...
                            Err(err) => {
                                ...
                                let proto = match err.kind {
                                    NetErrorKind::Proto(proto_err) => proto_err,
                                    _ => ProtoError::from(format!(
                                        "network error during DNSKEY lookup: {err}"
                                    )),
                                };
