...
use hickory_proto::op::{DnsRequest, DnsResponse, Message, MessageSigner, SerialMessage};
    ...
    async fn send(&self) -> Result<DnsResponse, NetError> {
        ...
        for _ in 0..3 {
            ...
            if msg_id != response.id() {
                ...
                return Err(NetErrorKind::BadTransactionId.into());
            }
                ...
                })
            {
                ...
                return Err(NetErrorKind::QueryCaseMismatch.into());
            }
            if !question_matches {
                warn!(
            ...
            if let Some(mut verifier) = verifier {
                return verifier(response_bytes).map_err(NetError::from);
            } else {
                return Ok(response);
            }
        }
...
mod tests {
    ...
    async fn retry_handler_test() -> Result<(), std::io::Error> {
        ...
        let ret = retry::<TokioRuntimeProvider>(
            FixedResponse {
                response: DnsResponse::from_message(message.clone()).map_err(NetError::from)?,
            },
