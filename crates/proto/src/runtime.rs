...
mod tokio_runtime {
    ...
    impl RuntimeProvider for TokioRuntimeProvider {
        ...
        fn connect_tls(
            ...
            Box::pin(async move {
                let AsyncIoTokioAsStd(tcp_stream) = tcp_stream;

                let early_data_enabled = client_config.enable_early_data;
                let tls_connector =
                    TlsConnector::from(client_config).early_data(early_data_enabled);
...
pub trait Spawn {
    ...
    ///
    /// This will periodically reap completed tasks, in order to prevent unbounded memory growth.
    fn spawn_bg<F>(&mut self, future: F)
    where
        F: Future<Output = Result<(), ProtoError>> + Send + 'static;
}
