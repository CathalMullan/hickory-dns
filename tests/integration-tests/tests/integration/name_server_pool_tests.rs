...
#[cfg(test)]
fn mock_nameserver(
    messages: Vec<Result<DnsResponse, NetError>>,
...
fn mock_nameserver_on_send_nx<O: OnSend + Unpin>(
    protocols: Vec<(ProtocolConfig, Vec<Result<DnsResponse, NetError>>)>,
    options: ResolverOpts,
    on_send: O,
    ip: IpAddr,
    trust_negative_responses: bool,
...
fn test_tcp_fallback_only_on_truncated() {
    ...
    match error.kind() {
        NetErrorKind::Proto(ProtoError {
            kind: ProtoErrorKind::Dns(DnsError::ResponseCode(ResponseCode::ServFail)),
            ..
        }) => {}
        kind => panic!(
...
fn test_tcp_fallback_on_io_error() {
    ...
    let tcp_nameserver = mock_nameserver(
        vec![
            DnsError::from_response(DnsResponse::from_message(tcp_message).unwrap())
    ...
    match error.kind() {
        NetErrorKind::Proto(ProtoError {
            kind: ProtoErrorKind::Dns(DnsError::ResponseCode(ResponseCode::NotImp)),
            ..
        }) => {}
        kind => panic!(
...
fn test_user_provided_server_order() {
    ...
    let to_dns_response = |records: Vec<Record>| -> Vec<Result<DnsResponse, NetError>> {
        records
            .iter()
            .map(|record| {
    ...
    preferred_server_responses.insert(0, Err(NetError::from(io::Error::other("fail"))));
...
fn test_return_error_from_highest_priority_nameserver() {
    ...
    match error.kind() {
        NetErrorKind::Proto(ProtoError {
            kind: ProtoErrorKind::Dns(DnsError::ResponseCode(response_code)),
            ..
